...
export const Route = createFileRoute("/controle")({
  ...
  head: () => ({
    meta: [
      { title: "Gestão — Finanças John e Thais" },
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="grid gap-3 sm:flex sm:flex-wrap sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm font-medium text-muted-foreground">Geração × consumo e rateio (uso exclusivo para cálculos)</p>
        ...
        <div className="flex flex-wrap gap-2 [&>*]:flex-1 sm:[&>*]:flex-none">
          ...
          {editing ? (
            ...
            <Button variant="outline" onClick={() => setEditing(true)} className="gap-2 border border-border bg-accent text-muted-foreground hover:text-foreground rounded-lg px-4 py-2 font-bold">
      ...
      <Card className="glass-card p-6">
        <div className="mb-6 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
          <Zap className="h-4 w-4 text-primary" /> Geração da Usina
        </div>
        <div className="grid grid-cols-1 gap-4 xs:grid-cols-2 md:grid-cols-4">
          <div>
            <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Nº de placas</Label>
            {editing ? (
              <Input type="number" value={config.panels_count} onChange={e => setConfig({ ...config, panels_count: Number(e.target.value) })} className="mt-1 border-border bg-input text-foreground" />
            ) : <div className="text-2xl font-bold text-foreground num-lg">{config.panels_count}</div>}

          </div>
          <div>
            <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">kW por placa/mês</Label>
            {editing ? (
              <Input type="number" step="0.01" value={config.kw_per_panel} onChange={e => setConfig({ ...config, kw_per_panel: Number(e.target.value) })} className="mt-1 border-border bg-input text-foreground" />
            ) : <div className="text-2xl font-bold text-foreground num-lg">{config.kw_per_panel} kW</div>}

          </div>
          <div>
            <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Geração total/mês</Label>
            <div className="text-2xl font-bold text-foreground num-lg">{totalGen.toLocaleString("pt-BR")} kW</div>

          </div>
          <div>
            <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Consumo total clientes</Label>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="glass-card p-5">
          <div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground light:text-emerald-600"><TrendingUp className="h-4 w-4 text-emerald-400 light:text-emerald-600" /> Saldo de Energia</div>
          <div className={`text-2xl font-bold num-lg ${saldo < 0 ? "text-red-400" : "text-emerald-400 light:text-emerald-600"}`}>{saldo >= 0 ? "+" : ""}{saldo.toLocaleString("pt-BR")} kW</div>
          <div className="mt-1 text-[10px] text-muted-foreground font-bold uppercase tracking-tight">{saldo >= 0 ? "Sobra de energia gerada" : "Consumo excede geração"}</div>
        </Card>
        <Card className="glass-card p-5">
          <div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"><BarChart3 className="h-4 w-4 text-muted-foreground" /> Aproveitamento</div>
          <div className="text-2xl font-bold text-foreground num-lg">{aproveitamento.toFixed(1)}%</div>
          <div className="mt-1 text-[10px] text-muted-foreground font-bold uppercase tracking-tight">Do total gerado utilizado</div>

        </Card>
        <Card className="glass-card p-5">
          <div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground light:text-emerald-600"><Zap className="h-4 w-4 text-emerald-400 light:text-emerald-600" /> Rateio total alocado</div>
          <div className={`text-2xl font-bold num-lg ${Math.abs(totalPct - 100) < 0.5 ? "text-emerald-400 light:text-emerald-600" : "text-red-400"}`}>
            {totalPct.toFixed(2)}%
          </div>
          <div className="mt-1 text-[10px] text-muted-foreground font-bold uppercase tracking-tight">
      ...
      <Card className="glass-card p-6 overflow-hidden">
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{rows.length} cliente(s)</span>
        ...
        <div className="overflow-x-auto -mx-6 px-6 scrollbar-hide">
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-border">
                <th className="pb-3 text-left font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Cliente</th>
                <th className="pb-3 text-left font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Unid. Consumidora</th>
                <th className="pb-3 text-center font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Rateio %</th>
                <th className="pb-3 text-right font-bold text-muted-foreground uppercase text-[10px] tracking-wider">kW Alocado/Mês</th>
                <th className="pb-3 text-center font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Consumo Médio</th>
                <th className="pb-3 text-right font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Saldo Cliente</th>
            ...
            <tbody>
              {rows.map((r, idx) => {
                ...
                return (
                  <tr key={r.client_id} className="border-b border-border last:border-0 hover:bg-accent transition-colors zebra-stripe">
                    ...
                    <td className="py-4 text-muted-foreground">{r.uc || "—"}</td>
                    <td className="py-4 text-center">
                      {editing
                        ? <Input className="mx-auto w-24 text-center border-border bg-input text-foreground" type="number" step="0.01" value={r.pct}
                    ...
                    <td className="py-4 text-center">
                      {editing
                        ? <Input className="mx-auto w-24 text-center border-border bg-input text-foreground" type="number" value={r.avg}
                    ...
                    <td className={`py-4 text-right font-bold num ${saldoCli < 0 ? "text-red-400" : "text-emerald-400 light:text-emerald-600"}`}>
            ...
            <tfoot className="border-t border-border bg-accent">
              <tr>
                ...
                <td className={`py-4 text-right font-bold num ${totalSaldo < 0 ? "text-red-400" : "text-emerald-400 light:text-emerald-600"}`}>
          ...
          {rows.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">Nenhum cliente ativo.</p>}
        </div>
      </Card>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="glass-card border-border text-foreground max-w-md">
        <DialogHeader><DialogTitle className="text-foreground dark:text-glow">Novo Cliente</DialogTitle></DialogHeader>

        <div className="space-y-3">
          <div><Label className="text-muted-foreground">Nome do cliente *</Label><Input value={f.name} onChange={e => setF({ ...f, name: e.target.value })} placeholder="Ex: JOÃO SILVA" className="bg-input border-border text-foreground" /></div>
          <div><Label className="text-muted-foreground">Unidade Consumidora (UC)</Label><Input value={f.uc_number} onChange={e => setF({ ...f, uc_number: e.target.value })} placeholder="Ex: 303007001223" className="bg-input border-border text-foreground" /></div>
          <div><Label className="text-muted-foreground">Consumo médio mensal (kW) *</Label><Input type="number" value={f.avg} onChange={e => setF({ ...f, avg: e.target.value })} placeholder="Ex: 500" className="bg-input border-border text-foreground" /></div>
          <div><Label className="text-muted-foreground">Rateio % *</Label><Input type="number" step="0.01" value={f.pct} onChange={e => setF({ ...f, pct: e.target.value })} placeholder="Ex: 8.20" className="bg-input border-border text-foreground" /></div>
