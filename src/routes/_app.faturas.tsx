...
function InvoiceDialog({ client, invoice, onClose }: { client: Client; invoice?: Invoice; onClose: () => void }) {
  ...
  const [f, setF] = useState({
    ...
    value_without_plant: invoice ? String(invoice.value_without_plant) : "",
    client_pays: invoice ? String(invoice.client_pays) : "",
    distributor_invoice: invoice ? String(invoice.distributor_invoice) : "",
    notes: invoice ? stripPaymentTag(invoice.notes) : "",
  });
  const [paymentStatus, setPaymentStatus] = useState<"pago" | "pendente">(invoice ? invoicePaymentStatus(invoice.notes) : "pago");
  const [saving, setSaving] = useState(false);
  const [file, setFile] = useState<File | null>(null);

  useEffect(() => {
    const cons = Number(f.consumption_kw) || 0;
    const price = Number(f.price_kw) || 0;
    const ilum = Number(f.public_lighting) || 0;
    const juros = Number(f.interest_fine) || 0;
    if (!invoice && cons && price) {
      const sem = cons * price + ilum + juros;
      const desconto = client.discount_pct / 100;
      const com = sem * (1 - desconto);
      setF((prev) => ({
        ...prev,
        value_without_plant: sem ? String(Math.round(sem * 100) / 100) : prev.value_without_plant,
        client_pays: com ? String(Math.round(com * 100) / 100) : prev.client_pays,
      }));
    }
  }, [f.consumption_kw, f.price_kw, f.public_lighting, f.interest_fine, client.discount_pct, invoice]);

  async function submit() {
    setSaving(true);
    let attachment_url = invoice?.attachment_url ?? null;
    if (file) {
      const path = `${client.id}/${f.reference_month}-${file.name}`;
      const up = await supabase.storage.from("faturas_v3_privado_v2").upload(path, file, { upsert: true });
      if (up.error) {
        setSaving(false);
    ...
    const payload = {
      ...
      uc_number: client.uc_number,
      consumption_kw: Number(f.consumption_kw) || 0,
      price_kw: Number(f.price_kw) || 0,
      ...
      client_pays: Number(f.client_pays) || 0,
      distributor_invoice: Number(f.distributor_invoice) || 0,
      notes: withPaymentTag(f.notes, paymentStatus),
      attachment_url,
    };
    const res = invoice
      ...
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input type="number" value={f.consumption_kw} onChange={(e) => setF({ ...f, consumption_kw: e.target.value })} />
          </div>
          <div>
            <Label>Preço kW</Label>
            <Input type="number" step="0.000001" value={f.price_kw} onChange={(e) => setF({ ...f, price_kw: e.target.value })} />
          </div>
          <div>
            <Label>Ilum. pública</Label>
          ...
          <div>
            <Label>Cliente pagou</Label>
            <Input type="number" step="0.01" value={f.client_pays} onChange={(e) => setF({ ...f, client_pays: e.target.value })} />
          </div>
          <div className="col-span-2">
            <Label>Fat. Concessionária</Label>
            <Input type="number" step="0.01" value={f.distributor_invoice} onChange={(e) => setF({ ...f, distributor_invoice: e.target.value })} />
          </div>
          <div className="col-span-2">
            <Label>Status de pagamento</Label>
            <Select value={paymentStatus} onValueChange={(v) => setPaymentStatus(v as "pago" | "pendente")}>
...
function HistoryDialog({ client, onClose }: { client: Client; onClose: () => void }) {
  ...
  const { data: invoices = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("invoices").select("*").eq("client_id", client.id).order("reference_date", { ascending: false });
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-5xl w-[95vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <Card className="p-4 border-none shadow-sm bg-accent">
              ...
              <div className="mt-1 text-lg font-bold text-emerald-600">{brl(totalClient)}</div>
            </Card>
            <Card className="p-4 border-none shadow-sm bg-accent">
              <div className="text-xs font-medium text-muted-foreground">Fat. Concessionária</div>
              <div className="mt-1 text-lg font-bold text-red-500">{brl(totalDistributor)}</div>
            </Card>
            <Card className="p-4 border-none shadow-sm bg-accent">
              <div className="text-xs font-medium text-muted-foreground">Lucro Bruto</div>
              <div className="mt-1 text-lg font-bold text-emerald-600">{brl(netProfit)}</div>
            </Card>
          </div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Button className="gap-2 text-white" style={{ backgroundColor: client.color }} onClick={() => setLaunching(true)} disabled={!client.active}>
              ...
              <table className="w-full table-fixed text-xs">
                <thead className="sticky top-0 z-10 bg-accent">
                  <tr className="text-[9px] uppercase tracking-wide text-muted-foreground">
                    <th className="w-[11%] px-1.5 py-2 text-left font-semibold">Mês</th>
                    <th className="w-[9%] px-1.5 py-2 text-right font-semibold">Consumo</th>
                    <th className="w-[12%] px-1.5 py-2 text-right font-semibold">S/ Usina</th>
                    <th className="w-[13%] px-1.5 py-2 text-right font-semibold">Cliente</th>
                    <th className="w-[13%] px-1.5 py-2 text-right font-semibold text-red-500">Concession.</th>
                    <th className="w-[12%] px-1.5 py-2 text-right font-semibold text-emerald-600">Lucro</th>
                    <th className="w-[7%] px-1 py-2 text-center font-semibold">Anexo</th>
                    <th className="w-[12%] px-1 py-2 text-center font-semibold">Status</th>
                  ...
                  {invoices.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} className="border-t border-border hover:bg-accent even:bg-accent/30">
                        <td className="px-1.5 py-2 text-left font-medium whitespace-nowrap">{monthLabelFromISO(inv.reference_date)}</td>
                        <td className="px-1.5 py-2 text-right text-muted-foreground tabular-nums">{Number(inv.consumption_kw).toLocaleString("pt-BR")}</td>
                        <td className="px-1.5 py-2 text-right tabular-nums">{brl(Number(inv.value_without_plant))}</td>
                        <td className="px-1.5 py-2 text-right text-emerald-600 tabular-nums">{brl(Number(inv.client_pays))}</td>
                        <td className="px-1.5 py-2 text-right text-red-500 tabular-nums">{brl(Number(inv.distributor_invoice))}</td>
                        <td className="px-1.5 py-2 text-right font-semibold text-emerald-600 tabular-nums">{brl(lucro)}</td>
                        <td className="px-1 py-2 text-center">{inv.attachment_url ? <Paperclip className="h-3.5 w-3.5 mx-auto text-muted-foreground" /> : <span className="text-muted-foreground">—</span>}</td>
                        ...
                        <td className="px-1 py-2 text-center">
                          <div className="inline-flex justify-center gap-0.5">
                            <button onClick={() => setEditing(inv)} className="p-1 text-muted-foreground hover:text-foreground" title="Editar"><Pencil className="h-3.5 w-3.5" /></button>
                            <button onClick={() => deleteInvoice(inv)} className="p-1 text-muted-foreground hover:text-red-500" title="Excluir"><Trash2 className="h-3.5 w-3.5" /></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      {editing && (
        <InvoiceDialog client={client} invoice={editing} onClose={() => { setEditing(null); qc.invalidateQueries({ queryKey: ["client-invoices", client.id] }); }} />
      )}
    </>
  );
}
