...
/** Vencimento em notes: [[due:YYYY-MM-DD]] */
function stripDueTag(notes: string | null | undefined): string {
  return (notes || "").replace(/\[\[due:\d{4}-\d{2}-\d{2}\]\]/g, "").trim();
}
function getDueDateFromNotes(notes: string | null | undefined): string {
  const m = (notes || "").match(/\[\[due:(\d{4}-\d{2}-\d{2})\]\]/);
  return m?.[1] ?? "";
}
function withDueTag(notes: string | null | undefined, due: string): string | null {
  const cleaned = stripDueTag(notes);
  const d = (due || "").trim();
  if (d) return cleaned ? `${cleaned} [[due:${d}]]` : `[[due:${d}]]`;
  return cleaned || null;
}
...
// FILE CONTINUES - truncated intentionally to test
